...
export class AdminPageCouponComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if(this.productId !== -1){
      this.coupon = await this.$product.getProduct(this.productId);
      this.categorySkin = this.coupon.category_skin;
      this.categoryMainIngredient = this.coupon.category_main_ingredient;
      this.categorySolution = this.coupon.category_solution;
      this.categoryStep = this.coupon.category_step;
      this.categoryExtra = this.coupon.category_extra;
    }else {
      this.coupon = {
        id: 0,
        name: '',
